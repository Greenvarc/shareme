...
function PinDetails({user}) {
  ...
  const fetchPinDetails=()=>{
    ...
    if (query){
      ...
      .then((data)=>{
        ...
        // similar pins
        if(data[0]){
        const  query=pinDetailMorePinQuery(pinId)
          client.fetch(query)
          .then((res)=>{
            setPins(res)
          })
  ...
  return (
  <>
    <div className='flex xl:flex-row flex-col m-auto bg-white' style={{maxWidth:'1400px',borderRadius:'32px'}}>
        ...
        <div className='max-h-370 overflow-y-auto'>
          {pinDetail?.comments?.map(({postedBy,comment,i})=>(
